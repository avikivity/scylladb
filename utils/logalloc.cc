...
class region_impl : public allocation_strategy {
    ...
    public:
        ...
        segment::size_type encoded_size() const {
            auto bits = log2ceil(_n); // 0 is illegal
            return (bits + 5) / 6;
        }
        ...
        // non-canoncial encoding to allow padding; encoded_size must be
        ...
        static object_descriptor decode_forwards(const char*& pos) {
            ...
            do {
                b = *p++;
                if (shift <= 32) {
                    // uncanonical encoding can cause large shift; undefined in C++
...
private:
    ...
    virtual size_t object_size_for_free(const void* obj) const noexcept override {
        auto pos = reinterpret_cast<const char*>(obj);
        auto desc = object_descriptor::decode_backwards(pos);
        return desc.migrator()->size(obj);
    }
