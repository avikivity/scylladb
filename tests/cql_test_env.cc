...
public:
    ...
    static future<> do_with(std::function<future<>(cql_test_env&)> func, cql_test_config cfg_in) {
        ...
        return seastar::async([cfg_in = std::move(cfg_in), func] {
            ...
            ss.start(std::ref(*db), std::ref(gms::get_gossiper()), std::ref(*auth_service), std::ref(cql_config), std::ref(sys_dist_ks), std::ref(*view_update_generator), std::ref(*feature_service), true, cfg_in.disabled_features).get();
