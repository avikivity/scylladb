...
column_condition::raw::prepare(database& db, const sstring& keyspace, const schema& schema, const column_definition& receiver) const {
    ...
    if (_collection_element) {
        ...
        collection_element_expression = prepare_expression(*_collection_element, db, keyspace, &schema, element_spec);
    }
    ...
    if (is_compare(_op)) {
        ...
        return column_condition::condition(receiver, std::move(collection_element_expression),
                prepare_expression(*_value, db, keyspace, &schema, value_spec), nullptr, _op);
    }
    ...
    if (_op == expr::oper_t::LIKE) {
        ...
        if (literal_term) {
            ...
            return column_condition::condition(receiver, std::move(collection_element_expression),
                    prepare_expression(*_value, db, keyspace, &schema, value_spec),
    ...
    if (_in_marker) {
        ...
        expr::expression multi_item_term = prepare_expression(*_in_marker, db, keyspace, &schema, value_spec);
    ...
    for (auto&& value : _in_values) {
        terms.push_back(prepare_expression(value, db, keyspace, &schema, value_spec));
    }
