...
get_set_intersection_function(data_dictionary::database db,
    ...
    if (!std::ranges::all_of(known_set_types, [] (data_type t) { return t != nullptr; })) {
        throw exceptions::invalid_request_exception("set_intersection() can only be called if all arguments are of set type");
    }
    ...
    return make_native_scalar_function<true>("set_intersection", set_type, {set_type, set_type},
        ...
        auto set_as_range = [&] (const bytes_opt& serialized_set, managed_bytes_view& buffer) {
            ...
            static_assert(std::convertible_to<listlike_partial_deserializing_iterator, listlike_partial_deserializing_iterator>);
        ...
        auto compare = [&] (managed_bytes_view_opt v1, managed_bytes_view_opt v2) {
            if (!v1 || !v2) {
