...
public:
    ...
    void construct_compaction(compaction_metadata& m) {
        if (!_ancestors.empty()) {
            m.ancestors.elements = utils::fragmented_vector<uint32_t>(_ancestors.begin(), _ancestors.end());
        }
        auto cardinality = _cardinality.get_bytes();
        m.cardinality.elements = utils::fragmented_vector<uint8_t>(cardinality.get(), cardinality.get() + cardinality.size());
    }
