...
class lazy_row {
    ...
    static inline row _empty_row;
private:
    template <typename Func, typename... Args>
    auto maybe_forward(Func func, Args&&... args) const {
        if (_row) {
            return std::invoke(func, _row.get(), std::forward<Args>(args)...);
        }
    }
    template <typename Func, typename... Args>
    auto maybe_forward(Func func, Args&&... args) {
        if (_row) {
            return std::invoke(func, _row.get(), std::forward<Args>(args)...);
        }
    }
    template <typename Ret, typename Func, typename... Args>
    Ret maybe_forward_ret(Ret ret_if_null, Func func, Args&&... args) {
        if (_row) {
            return std::invoke(_row.get(), std::forward<Args>(args)...);
        } else {
            return ret_if_null;
        }
    }
    template <typename Ret, typename Func, typename... Args>
    Ret maybe_forward_ret(Ret ret_if_null, Func func, Args&&... args) const {
        if (_row) {
            return std::invoke(_row.get(), std::forward<Args>(args)...);
        } else {
            return ret_if_null;
    ...
    template <typename Func, typename... Args>
    auto emplace_and_forward(Func func, Args&&... args) {
        maybe_create();
        std::invoke(func, _row.get(), std::forward<Args>(args)...);
    }
public:
    lazy_row() = default;
    ...
    size_t external_memory_usage(const schema& s, column_kind kind) const {
        ...
        return current_allocator().object_memory_size_in_allocator(_row.get())
                + _row->external_memory_usage(s, kind);
    }
