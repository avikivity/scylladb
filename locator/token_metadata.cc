...
std::vector<nonwrapping_range<token>> token_metadata::get_primary_ranges_for(std::unordered_set<token> tokens) {
    ...
    for (auto right : tokens) {
        ...
        if (left < right) {
            ranges.emplace_back(nonwrapping_range<token>::bound(std::move(left), false),
                    nonwrapping_range<token>::bound(right, true));
        } else {
            ranges.emplace_back(
                    nonwrapping_range<token>::bound(std::move(left), false),
                    stdx::nullopt);
            ranges.emplace_back(
                    stdx::nullopt,
                    nonwrapping_range<token>::bound(right, true));
