...
#include "db_clock.hh"
#include "data_dictionary/database.hh"
...
public:
    void set_keyspace(data_dictionary::database db, std::string_view keyspace);
    ...
    future<> has_keyspace_access(data_dictonary::database db, const sstring&, auth::permission) const;
    future<> has_column_family_access(data_dictonary::database db, const sstring&, const sstring&, auth::permission,
                                      auth::command_desc::type = auth::command_desc::type::OTHER) const;
    future<> has_schema_access(data_dictonary::database db, const schema& s, auth::permission p) const;

private:
    future<> has_access(data_dictonary::database db, const sstring& keyspace, auth::command_desc) const;
