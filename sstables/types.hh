...
struct filter {
    ...
    explicit filter(int hashes, utils::fragmented_vector<uint64_t> buckets) : hashes(hashes), buckets({std::move(buckets)}) {}
};
...
struct summary_ka {
    ...
    utils::fragmented_vector<uint32_t> positions;   // can be large, so use a deque instead of a vector
    utils::fragmented_vector<summary_entry> entries;
...
struct checksum {
    uint32_t chunk_size;
    utils::fragmented_vector<uint32_t> checksums;
