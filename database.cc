...
#define SEASTAR_NODISCARD 
           ...
           public:        bool available() const noexcept ;
             ...
             if (available() && !need_preempt())               if (failed())                 return futurator::make_exception_future(                 static_cast<future_state_base &&>(get_available_state_ref()));
                                     }
           };
                                               namespace internal {
       ...
       int & reader;
             return do_with(           std::move, [&reader, timeout](Consumer &c) -> future<> {
                 return repeat([&reader, &c, timeout] {                  return read_mutation_from_flat_mutation_reader(reader, timeout)                   .then;                }
    );
               }
      );
           }
      ...
      ,           cql_serialization_format::internal(), query::max_rows);
             return do_with(           std::move(slice), std::move(m), std::vector<locked_cell>(),           [this, &cf, timeout](const query::partition_slice &slice, mutation &m,                                std::vector<locked_cell> &locks) mutable {
                 return cf.lock_counter_cells(m, timeout)                 .then([&, timeout, this](std::vector<locked_cell> lcs) mutable {                      return counter_write_query(schema_ptr(), mutation_source(),                                              m.decorated_key(), slice, nullptr)                       .then([this, &cf, &m, timeout](auto mopt) {                            return std::move(m);                          }   );                    }
    );
               }
      );
           }
