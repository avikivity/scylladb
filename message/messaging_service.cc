...
// STREAM_RECEIVE_RANGE_SYNC
void messaging_service::register_stream_receive_range_sync(std::function<future<> (const rpc::client_info& cinfo, UUID plan_id, sstring description, UUID table, dht::token_range_vector)>&& func) {
    register_handler(this, messaging_verb::STREAM_RECEIVE_RANGE_SYNC, std::move(func));
}
void messaging_service::unregister_stream_receive_range_sync() {
    _rpc->unregister_handler(messaging_verb::STREAM_RECEIVE_RANGE_SYNC);
}
future<> messaging_service::send_stream_receive_range_sync(msg_addr id, UUID plan_id, sstring description, UUID table, dht::token_range_vector ranges) {
    return send_message<void>(this, messaging_verb::STREAM_RECEIVE_RANGE_SYNC, id, plan_id, description, table, std::move(ranges));
}
