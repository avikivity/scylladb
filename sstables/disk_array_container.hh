...
// A container for `disk_array`. Requirements are fast random access, the
// ability to append at the end, and limited contiguous allocations. Otherwise
// it's not a real STL container.
//
...
struct disk_array_container_free_deleter {
    void operator()(void* x) const { ::free(x); }
};

template <typename T, size_t max_contiguous_allocation = 128*1024>
class disk_array_container {
    static_assert(std::is_nothrow_move_constructible<T>::value, "T must be nothrow move constructible");
    using chunk_ptr = std::unique_ptr<T[], disk_array_container_free_deleter>;
...
private:
    ...
    void check_bounds(size_t i) const {
        if (i >= _size) {
            throw std::out_of_range("disk_array_container out of range access");
        }
    }
...
public:
    disk_array_container() = default;
    disk_array_container(const disk_array_container& x);
    disk_array_container(disk_array_container&& x) noexcept;
    template <typename Iterator>
    disk_array_container(Iterator begin, Iterator end);
    explicit disk_array_container(size_t n, const T& value = T());
    ~disk_array_container();
    disk_array_container& operator=(const disk_array_container& x);
    disk_array_container& operator=(disk_array_container&& x) noexcept;
...
public:
    ...
    public:
        ...
        friend class disk_array_container;
    };
...
public:
    bool operator==(const disk_array_container& x) const {
...
disk_array_container<T, max_contiguous_allocation>::disk_array_container(const disk_array_container& x) {
    reserve(x.size());
    for (auto&& e : x) {
        push_back(e);
    }
...
template <typename T, size_t max_contiguous_allocation>
disk_array_container<T, max_contiguous_allocation>::disk_array_container(disk_array_container&& x) noexcept
...
template <typename T, size_t max_contiguous_allocation>
template <typename Iterator>
disk_array_container<T, max_contiguous_allocation>::disk_array_container(Iterator begin, Iterator end) {
...
template <typename T, size_t max_contiguous_allocation>
disk_array_container<T, max_contiguous_allocation>::disk_array_container(size_t n, const T& value) {
    reserve(n);
...
template <typename T, size_t max_contiguous_allocation>
disk_array_container<T, max_contiguous_allocation>&
disk_array_container<T, max_contiguous_allocation>::operator=(const disk_array_container& x) {
    auto tmp = disk_array_container(x);
...
inline
disk_array_container<T, max_contiguous_allocation>&
disk_array_container<T, max_contiguous_allocation>::operator=(disk_array_container&& x) noexcept {
    if (this != &x) {
        this->~disk_array_container();
        new (this) disk_array_container(std::move(x));
    }
    return *this;
}

template <typename T, size_t max_contiguous_allocation>
disk_array_container<T, max_contiguous_allocation>::~disk_array_container() {
...
template <typename T, size_t max_contiguous_allocation>
typename disk_array_container<T, max_contiguous_allocation>::chunk_ptr
disk_array_container<T, max_contiguous_allocation>::new_chunk(size_t n) {
...
template <typename T, size_t max_contiguous_allocation>
void
disk_array_container<T, max_contiguous_allocation>::migrate(T* begin, T* end, T* result) {
    while (begin != end) {
...
template <typename T, size_t max_contiguous_allocation>
void
disk_array_container<T, max_contiguous_allocation>::make_room(size_t n) {
...
template <typename T, size_t max_contiguous_allocation>
void
disk_array_container<T, max_contiguous_allocation>::do_reserve_for_push_back() {
    if (_capacity == 0) {
...
template <typename T, size_t max_contiguous_allocation>
void
disk_array_container<T, max_contiguous_allocation>::resize(size_t n) {
    reserve(n);
...
template <typename T, size_t max_contiguous_allocation>
void
disk_array_container<T, max_contiguous_allocation>::shrink_to_fit() {
    if (_chunks.empty()) {
        return;
    }
...
template <typename T, size_t max_contiguous_allocation>
void
disk_array_container<T, max_contiguous_allocation>::clear() {
    resize(0);
}
