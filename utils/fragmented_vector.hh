...
struct fragmented_vector_free_deleter {
...
class fragmented_vector {
    ...
    using chunk_ptr = std::unique_ptr<T[], fragmented_vector_free_deleter>;
...
private:
    ...
    void check_bounds(size_t i) const {
        if (i >= _size) {
            throw std::out_of_range("fragmented_vector out of range access");
        }
    }
    void clean_up();
...
public:
    fragmented_vector() = default;
    fragmented_vector(const fragmented_vector& x);
    fragmented_vector(fragmented_vector&& x) noexcept;
    template <typename Iterator>
    fragmented_vector(Iterator begin, Iterator end);
    explicit fragmented_vector(size_t n, const T& value = T());
    ~fragmented_vector();
    fragmented_vector& operator=(const fragmented_vector& x);
    fragmented_vector& operator=(fragmented_vector&& x) noexcept;
...
public:
    bool operator==(const fragmented_vector& x) const {
...
fragmented_vector<T, max_contiguous_allocation>::fragmented_vector(const fragmented_vector& x) {
    reserve(x.size());
    try {
        for (auto&& e : x) {
            push_back(e);
...
template <typename T, size_t max_contiguous_allocation>
fragmented_vector<T, max_contiguous_allocation>::fragmented_vector(fragmented_vector&& x) noexcept
...
template <typename T, size_t max_contiguous_allocation>
template <typename Iterator>
fragmented_vector<T, max_contiguous_allocation>::fragmented_vector(Iterator begin, Iterator end) {
...
template <typename T, size_t max_contiguous_allocation>
fragmented_vector<T, max_contiguous_allocation>::fragmented_vector(size_t n, const T& value) {
    reserve(n);
    try {
...
template <typename T, size_t max_contiguous_allocation>
fragmented_vector<T, max_contiguous_allocation>&
fragmented_vector<T, max_contiguous_allocation>::operator=(const fragmented_vector& x) {
    auto tmp = fragmented_vector(x);
...
inline
fragmented_vector<T, max_contiguous_allocation>&
fragmented_vector<T, max_contiguous_allocation>::operator=(fragmented_vector&& x) noexcept {
    if (this != &x) {
        this->~fragmented_vector();
        new (this) fragmented_vector(std::move(x));
    }
    return *this;
}

template <typename T, size_t max_contiguous_allocation>
fragmented_vector<T, max_contiguous_allocation>::~fragmented_vector() {
    clean_up();
}

template <typename T, size_t max_contiguous_allocation>
void
fragmented_vector<T, max_contiguous_allocation>::clean_up() {
...
template <typename T, size_t max_contiguous_allocation>
typename fragmented_vector<T, max_contiguous_allocation>::chunk_ptr
fragmented_vector<T, max_contiguous_allocation>::new_chunk(size_t n) {
...
template <typename T, size_t max_contiguous_allocation>
void
fragmented_vector<T, max_contiguous_allocation>::migrate(T* begin, T* end, T* result) {
    while (begin != end) {
...
template <typename T, size_t max_contiguous_allocation>
void
fragmented_vector<T, max_contiguous_allocation>::make_room(size_t n) {
...
template <typename T, size_t max_contiguous_allocation>
void
fragmented_vector<T, max_contiguous_allocation>::do_reserve_for_push_back() {
    if (_capacity == 0) {
...
template <typename T, size_t max_contiguous_allocation>
void
fragmented_vector<T, max_contiguous_allocation>::resize(size_t n) {
    reserve(n);
...
template <typename T, size_t max_contiguous_allocation>
void
fragmented_vector<T, max_contiguous_allocation>::shrink_to_fit() {
    if (_chunks.empty()) {
        return;
    }
...
template <typename T, size_t max_contiguous_allocation>
void
fragmented_vector<T, max_contiguous_allocation>::clear() {
    resize(0);
}
