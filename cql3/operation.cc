...
operation::set_element::prepare(database& db, const sstring& keyspace, const schema& schema, const column_definition& receiver) const {
    ...
    if (rtype->get_kind() == abstract_type::kind::list) {
        auto&& lval = prepare_expression(_value, db, keyspace, &schema, lists::value_spec_of(*receiver.column_specification));
        if (_by_uuid) {
            auto&& idx = prepare_expression(_selector, db, keyspace, &schema, lists::uuid_index_spec_of(*receiver.column_specification));
            return make_shared<lists::setter_by_uuid>(receiver, std::move(idx), std::move(lval));
        } else {
            auto&& idx = prepare_expression(_selector, db, keyspace, &schema, lists::index_spec_of(*receiver.column_specification));
        ...
        auto key = prepare_expression(_selector, db, keyspace, &schema, maps::key_spec_of(*receiver.column_specification));
        auto mval = prepare_expression(_value, db, keyspace, &schema, maps::value_spec_of(*receiver.column_specification));
...
shared_ptr<operation>
operation::set_field::prepare(database& db, const sstring& keyspace, const schema& schema, const column_definition& receiver) const {
    ...
    auto val = prepare_expression(_value, db, keyspace, &schema, user_types::field_spec_of(*receiver.column_specification, *idx));
...
shared_ptr<operation>
operation::field_deletion::prepare(database& db, const sstring& keyspace, const schema& schema, const column_definition& receiver) const {
    if (!receiver.type->is_user_type()) {
        throw exceptions::invalid_request_exception(
...
shared_ptr<operation>
operation::addition::prepare(database& db, const sstring& keyspace, const schema& schema, const column_definition& receiver) const {
    auto v = prepare_expression(_value, db, keyspace, &schema, receiver.column_specification);
...
shared_ptr<operation>
operation::subtraction::prepare(database& db, const sstring& keyspace, const schema& schema, const column_definition& receiver) const {
    auto ctype = dynamic_pointer_cast<const collection_type_impl>(receiver.type);
    if (!ctype) {
        if (!receiver.is_counter()) {
            throw exceptions::invalid_request_exception(format("Invalid operation ({}) for non counter column {}", to_string(receiver), receiver.name()));
    ...
    if (ctype->get_kind() == abstract_type::kind::list) {
        return make_shared<lists::discarder>(receiver, prepare_expression(_value, db, keyspace, &schema, receiver.column_specification));
    } else if (ctype->get_kind() == abstract_type::kind::set) {
        return make_shared<sets::discarder>(receiver, prepare_expression(_value, db, keyspace, &schema, receiver.column_specification));
    } else if (ctype->get_kind() == abstract_type::kind::map) {
        ...
        return ::make_shared<sets::discarder>(receiver, prepare_expression(_value, db, keyspace, &schema, std::move(vr)));
    }
    abort();
}
...
shared_ptr<operation>
operation::prepend::prepare(database& db, const sstring& keyspace, const schema& schema, const column_definition& receiver) const {
    auto v = prepare_expression(_value, db, keyspace, &schema, receiver.column_specification);
...
::shared_ptr <operation>
operation::set_value::prepare(database& db, const sstring& keyspace, const schema& schema, const column_definition& receiver) const {
    auto v = prepare_expression(_value, db, keyspace, &schema, receiver.column_specification);
...
::shared_ptr <operation>
operation::set_counter_value_from_tuple_list::prepare(database& db, const sstring& keyspace, const schema& schema, const column_definition& receiver) const {
    ...
    auto v = prepare_expression(_value, db, keyspace, &schema, spec);
...
shared_ptr<operation>
operation::element_deletion::prepare(database& db, const sstring& keyspace, const schema& schema, const column_definition& receiver) const {
    if (!receiver.type->is_collection()) {
    ...
    if (ctype->get_kind() == abstract_type::kind::list) {
        auto&& idx = prepare_expression(_element, db, keyspace, &schema, lists::index_spec_of(*receiver.column_specification));
        return make_shared<lists::discarder_by_index>(receiver, std::move(idx));
    } else if (ctype->get_kind() == abstract_type::kind::set) {
        auto&& elt = prepare_expression(_element, db, keyspace, &schema, sets::value_spec_of(*receiver.column_specification));
        return make_shared<sets::element_discarder>(receiver, std::move(elt));
    } else if (ctype->get_kind() == abstract_type::kind::map) {
        auto&& key = prepare_expression(_element, db, keyspace, &schema, maps::key_spec_of(*receiver.column_specification));
