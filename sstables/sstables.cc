...
parse(random_access_reader& in, Size& len, disk_array_container<Members>& arr) {
...
write(file_writer& out, const disk_array_container<Members>& arr) {
    for (auto& a : arr) {
        write(out, a);
    }
}
...
future<> parse(random_access_reader& in, summary& s) {
    ...
    return parse(in, s.header.min_index_interval,
        ...
        return in.read_exactly(s.header.size * sizeof(pos_type)).then([&in, &s] (auto buf) {
            ...
            s.positions = disk_array_container<pos_type>(nr, nr + s.header.size);
...
inline void write(file_writer& out, const utils::streaming_histogram& sh) {
    ...
    a.elements = boost::copy_range<disk_array_container<streaming_histogram_element>>(sh.bin
