...
std::unique_ptr<attributes> attributes::raw::prepare(database& db, const sstring& ks_name, const sstring& cf_name) const {
    ...
    if (timestamp.has_value()) {
        ts = prepare_expression(*timestamp, db, ks_name, /* schema */ nullptr, timestamp_receiver(ks_name, cf_name));
    }

    if (time_to_live.has_value()) {
        ttl = prepare_expression(*time_to_live, db, ks_name, /* schema */ nullptr, time_to_live_receiver(ks_name, cf_name));
    }

    if (timeout.has_value()) {
        to = prepare_expression(*timeout, db, ks_name, /* schema */ nullptr, timeout_receiver(ks_name, cf_name));
    }
