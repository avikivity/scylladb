...
struct filter {
    ...
    explicit filter(int hashes, disk_array_container<uint64_t> buckets) : hashes(hashes), buckets({std::move(buckets)}) {}
};
...
struct summary_ka {
    ...
    disk_array_container<uint32_t> positions;   // can be large, so use a deque instead of a vector
    disk_array_container<summary_entry> entries;
...
struct checksum {
    uint32_t chunk_size;
    disk_array_container<uint32_t> checksums;
