...
private:
    ...
    static void ranges_opt(std::vector<nonwrapping_range<dht::token>>& var,
        ...
        for (auto range : range_strings) {
            ...
            if (rng.is_wrap_around(dht::token_comparator())) {
                auto unw = rng.unwrap();
                var.push_back(nonwrapping_range<dht::token>(std::move(unw.first)));
                var.push_back(nonwrapping_range<dht::token>(std::move(unw.second)));
            } else {
                var.emplace_back(nonwrapping_range<dht::token>(std::move(rng)));
            }
        }
        options.erase(it);
    }
