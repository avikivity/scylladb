...
static
value_set
solve(const analyzed_column& ac, const query_options& options) {
    if (ac.solve_for_column) {
        return ac.solve_for_column(options);
    }

    return possible_column_values(ac.col, ac.filter, options);
}
...
static
analyzed_column
replace_column_def(const analyzed_column& a, const column_definition* col) {
    return analyzed_column{
        .solve_for_column = a.solve_for_column,  // Note: does not replace and `col` embedded in the function
...
static std::vector<analyzed_column> extract_clustering_prefix_restrictions(
        const expr::expression& where_clause, schema_ptr schema) {
    using namespace expr;
    ...
    struct visitor {
        schema_ptr table_schema;
        std::vector<analyzed_column> multi; ///< All multi-column restrictions.
        /// All single-clustering-column restrictions, grouped by column.  Each value is either an atom or a
        /// conjunction of atoms.
        std::unordered_map<const column_definition*, analyzed_column> single;
        const binary_operator* current_binary_operator = nullptr;
        ...
        void operator()(const tuple_constructor& tc) {
            ...
            with_current_binary_operator(*this, [&] (const binary_operator& b) {
                multi.push_back(analyzed_column{
                    .solve_for_column = nullptr, // FIXME: implement
                    .filter = b,
                    .col = nullptr,  // Since it's multi-column...
                    .is_singleton = false,
                });
            });
        }
        ...
        void operator()(const subscript& sub) {
            ...
            with_current_binary_operator(*this, [&] (const binary_operator& b) {
                if (cval.col->is_clustering_key()) {
                    auto a = analyzed_column{
                        .solve_for_column = std::bind_front(possible_column_values, cval.col, b),
                        .filter = b,
                        .col  = cval.col,
                        .is_singleton = b.op == oper_t::EQ,
                    };
    ...
    std::vector<analyzed_column> prefix;
...
std::vector<query::clustering_range> get_multi_column_clustering_bounds(
        const query_options& options,
        schema_ptr schema,
        const std::vector<analyzed_column>& multi_column_restrictions) {
    multi_column_range_accumulator acc{options, schema};
    for (const auto& restr : multi_column_restrictions | std::views::transform(&analyzed_column::filter)) {
...
static std::vector<query::clustering_range> get_index_v1_token_range_clustering_bounds(
        ...
        const analyzed_column& token_restriction) {
    ...
    analyzed_column new_token_restrictions = replace_column_def(token_restriction, &token_column_bigint);

    std::variant<value_list, interval<managed_bytes>> values =
        new_token_restrictions.solve_for_column(options);
...
query::clustering_range range_from_raw_bounds(
        const std::vector<analyzed_column>& exprs, const query_options& options, const schema& schema) {
    opt_bound lb, ub;
    for (const auto& e : exprs | std::views::transform(&analyzed_column::filter)) {
...
statement_restrictions::build_get_clustering_bounds_fn() const {
    ...
    if (find_binop(_clustering_prefix_restrictions[0].filter, is_multi_column)) { // FIXME: adjust for solve_for_column
      return [&] (const query_options& options) -> std::vector<query::clustering_range> {
        bool all_natural = true, all_reverse = true; ///< Whether column types are reversed or natural.
        for (auto& r : _clustering_prefix_restrictions | std::views::transform(&analyzed_column::filter)) { // TODO: move to constructor, do only once.
            using namespace expr;
...
void statement_restrictions::prepare_indexed_global(const schema& idx_tbl_schema) {
    ...
    if (has_token_restrictions()) {
        ...
        _idx_tbl_ck_prefix = std::vector{analyzed_column{
            .solve_for_column = nullptr, // FIXME: adjust for solve_for_column
    ...
    _idx_tbl_ck_prefix = std::vector<analyzed_column>(1 + _schema->partition_key_size(), analyzed_column{
        .solve_for_column = nullptr,  // FIXME: this is all overwritten later. Should be refactored.
    ...
    if (single_column_partition_key_restrictions) {
      for (const auto& e : single_column_partition_key_restrictions->per_column_restrictions) {
        const auto col = e.col;
        const auto pos = _schema->position(*col) + 1;
        (*_idx_tbl_ck_prefix)[pos] = replace_column_def(e, &idx_tbl_schema.clustering_column_at(pos));
      }
    }

    if (std::ranges::any_of(*_idx_tbl_ck_prefix | std::views::drop(1) | std::views::transform(&analyzed_column::filter), is_empty_restriction)) {
    ...
    auto pk_expressions = (*_idx_tbl_ck_prefix)
            | std::views::transform(&analyzed_column::filter)
    ...
    auto pk_solvers = (*_idx_tbl_ck_prefix)
            | std::views::drop(1) // skip the token restriction
            | std::views::take(_schema->partition_key_size()) // take only the partition key restrictions
            | std::views::transform(&analyzed_column::solve_for_column)
            | std::ranges::to<std::vector>();
    ...
    (*_idx_tbl_ck_prefix)[0] = analyzed_column{
        .solve_for_column = std::move(token_solver),
...
void statement_restrictions::prepare_indexed_local(const schema& idx_tbl_schema) {
    ...
    _idx_tbl_ck_prefix->push_back(analyzed_column{
        .solve_for_column = std::bind_front(possible_column_values, &indexed_column, replaced_idx_restriction),
...
void statement_restrictions::add_clustering_restrictions_to_idx_ck_prefix(const schema& idx_tbl_schema) {
    for (const auto& e : _clustering_prefix_restrictions) {
        ...
        auto a = analyzed_column{
            .solve_for_column = std::bind_front(possible_column_values, col_in_index, replaced),
            .filter = replaced,
            .col = col_in_index,
            .is_singleton = false, // FIXME: could be a singleton token. Not very important.
        };
...
void statement_restrictions::validate_primary_key(const query_options& options) const {
    ...
    validate_primary_key_restrictions(options, _clustering_prefix_restrictions | std::views::transform(&analyzed_column::filter));
}
