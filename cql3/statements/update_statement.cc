    ...
    prepare_context& ctx, std::unique_ptr<attributes> attrs, cql_stats& stats) const
{
    ...
    for (size_t i = 0; i < _column_names.size(); i++) {
        ...
        if (def->is_primary_key()) {
            ...
            auto operation = operation::set_value(value).prepare(db, keyspace(), *schema, *def);
    ...
    prepare_context& ctx, std::unique_ptr<attributes> attrs, cql_stats& stats) const
{
    ...
    // We pass null schema since inserted values can't reference the table
    auto prepared_json_value = prepare_expression(_json_value, db, "", /* schema */ nullptr, make_lw_shared<column_specification>("", "", json_column_placeholder, utf8_type));
    ...
    prepare_context& ctx, std::unique_ptr<attributes> attrs, cql_stats& stats) const
{
    ...
    for (auto&& entry : _updates) {
        ...
        auto operation = entry.second->prepare(db, keyspace(), *schema, *def);
        operation->fill_prepare_context(ctx);
