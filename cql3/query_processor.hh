...
public:
    ...
    {
        ParsedStatement.Prepared prepared_statement = internalStatements.get(query);
        if (prepared_statement != null)
            return prepared_statement;

        // Note: if 2 threads prepare the same query, we'll live so don't bother synchronizing
        prepared_statement = parseStatement(query, internalQueryState());
        prepared_statement.statement.validate(internalQueryState().getClientState());
        internalStatements.putIfAbsent(query, prepared_statement);
        return prepared_statement;
    }
#endif
private:
