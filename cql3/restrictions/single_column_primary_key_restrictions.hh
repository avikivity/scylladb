...
#include "restrictions_config.hh"
...
inline
void check_cartesian_product_size(size_t size, size_t max) {
    if (size > max) {
        throw std::runtime_error(fmt::format("Primary key cartesian product size {} is greater than maximum {}",
                size, max));
    }
}
