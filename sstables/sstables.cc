...
parse(random_access_reader& in, Size& len, utils::fragmented_vector<Members>& arr) {
...
write(file_writer& out, const utils::fragmented_vector<Members>& arr) {
    for (auto& a : arr) {
        write(out, a);
    }
}
...
future<> parse(random_access_reader& in, summary& s) {
    ...
    return parse(in, s.header.min_index_interval,
        ...
        return in.read_exactly(s.header.size * sizeof(pos_type)).then([&in, &s] (auto buf) {
            ...
            s.positions = utils::fragmented_vector<pos_type>(nr, nr + s.header.size);
...
inline void write(file_writer& out, const utils::streaming_histogram& sh) {
    ...
    a.elements = boost::copy_range<utils::fragmented_vector<streaming_histogram_element>>(sh.bin
