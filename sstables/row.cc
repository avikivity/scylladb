...
data_consume_context sstable::data_consume_rows(
    ...
    return std::make_unique<data_consume_context::impl>(shared_from_this(),
            consumer, data_stream(toread.start, data_size() - toread.start,
                consumer.io_priority(), _partition_range_history), toread.start, toread.end - toread.start, toread.ri, sg);
}
...
data_consume_context sstable::data_consume_single_partition(
    ...
    return std::make_unique<data_consume_context::impl>(shared_from_this(),
            consumer, data_stream(toread.start, toread.end - toread.start,
                 consumer.io_priority(), _single_partition_history), toread.start, toread.end - toread.start, toread.ri, sg);
}


data_consume_context sstable::data_consume_rows(row_consumer& consumer, seastar::scheduling_group sg) {
    return data_consume_rows(consumer, {0, data_size()}, sg);
}
